...
{ H53: fuse the four Laurent child shifts and fold directly into the kept half. }
{ A and B use the same kernel recursion and folding; only bit storage differs. }
...
for i0:=0 to High(src) do if src[i0] then
  ...
  if p<=limit then dst[p]:=not dst[p];
  end;
end;
