...
class LightsOut(Scene):
    def construct(self):
        ...
        show_subtitle(self, "这里，粉色原点是用按钮表达按钮，浅蓝色方块是用按钮表达灯。")
        ...
        for y in range(rows):
            for x in range(cols):
                ...
                G5_[y][x] = make_grid(self, w=cols * rows, h=1, w_l=1, h_l=1, lgt_x=3, btn_x=0, lgt_y=my, btn_y=my, sz=sz, rt=0.01, mat=[mat_l[idx][:]], mat_l=[[1]], show=True)
        ...
        press(self, G5, 0,0)
        trans_grid(self,G5,G5_[0][0], rt=1.0);
        press(self, G5, 0,0)
        press(self, G5, 1,0)
        trans_grid(self,G5,G5_[1][0], rt=1.0);
