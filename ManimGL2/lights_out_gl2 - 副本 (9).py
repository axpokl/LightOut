...
def add_cell(scene, G_from, G_to, sx, sy, tx, ty, rt=1.0):
...
def make_mat_k(n):
    ...
    # 第一层：直接放单位矩阵（第一行是单位向量）
    for y in range(n):
        for x in range(n):
            M[0][y][x] = I[y][x]

    # 后续层：通用递推；k=1 时用 B=Z（相当于“没有更上面一层”）
...
#演示n=5,7,11
        ...
        for k in range(rows):
            for y in range(cols):
                ...
                G5Y_[k][y] = make_grid(self, w=1, h=1, lgt_x=mx, btn_x=mx, lgt_y=my, btn_y=my, sz=sz, mat=[[0]*cols for _ in range(rows)], mat_l=[[MAT5KY[k][y]]], show=True, rt=0.05)
...
#圈出x1-5,然后消失
        ...
        add_cell(self, G5_[0][0], G5_[1][0], 0, 0, 0, 0)
        del_cells(self, [G5_[0][0]], which="btn", indices=[(0, 0)])
        add_cell(self, G5_[1][0], G5_[1][0], 0, 0, 0, 0)
        del_cells(self, [G5_[1][0]], which="btn", indices=[(0, 0)])
        add_cell(self, G5_[1][0], G5_[1][0], 1, 0, 1, 0)
        del_cells(self, [G5_[1][0]], which="btn", indices=[(1, 0)])
        add_cell(self, G5_[1][1], G5_[1][0], 0, 0, 0, 0)
        del_cells(self, [G5_[1][1]], which="btn", indices=[(0, 0)])
        add_cell(self, G5_[1][1], G5_[1][0], 1, 0, 1, 0)
        del_cells(self, [G5_[1][1]], which="btn", indices=[(1, 0)])
        add_cell(self, G5_[1][1], G5_[1][0], 2, 0, 2, 0)
        ...
        add_cell(self, G5_[0][1], G5_[1][1], 1, 0, 1, 0)
        del_cells(self, [G5_[0][1]], which="btn", indices=[(1, 0)])
        add_cell(self, G5_[1][0], G5_[1][1], 0, 0, 0, 0)
        del_cells(self, [G5_[1][0]], which="btn", indices=[(0, 0)])
        add_cell(self, G5_[1][0], G5_[1][1], 1, 0, 1, 0)
        del_cells(self, [G5_[1][0]], which="btn", indices=[(1, 0)])
        add_cell(self, G5_[1][1], G5_[1][1], 0, 0, 0, 0)
        del_cells(self, [G5_[1][1]], which="btn", indices=[(0, 0)])
        add_cell(self, G5_[1][1], G5_[1][1], 1, 0, 1, 0)
        del_cells(self, [G5_[1][1]], which="btn", indices=[(1, 0)])
        add_cell(self, G5_[1][1], G5_[1][1], 2, 0, 2, 0)
        del_cells(self, [G5_[1][1]], which="btn", indices=[(2, 0)])
        add_cell(self, G5_[1][2], G5_[1][1], 1, 0, 1, 0)
        del_cells(self, [G5_[1][2]], which="btn", indices=[(1, 0)])
        add_cell(self, G5_[1][2], G5_[1][1], 2, 0, 2, 0)
        del_cells(self, [G5_[1][2]], which="btn", indices=[(2, 0)])
        add_cell(self, G5_[1][2], G5_[1][1], 3, 0, 3, 0)
        del_cells(self, [G5_[1][2]], which="btn", indices=[(3, 0)])
        ...
        for k in range(rows):
            for y in range(cols):
                ...
                gridY = G5Y_[k][y]
                row_y = MAT5KY[k][y]
                if row_y and not gridY["lgt"][0][0]:
