...
#演示n=5,7,11
        ...
        self.wait(2)
        show_subtitle(self, "1. 使用零向量：第一次翻转发生在从一到二行的迭代过程中，", "在此之前没有发生过翻转，因此Y的第一行是全零。")
        ...
        LAT1_1 = show_latex(self, "<cY>B(x-1,y)⊕B(x+1,y)⊕B(x,y-1)⊕B(x,y+1)=0", 0, 2.0)
        self.wait(2)
        ...
        hl_cells(self, [G5_[1][1]], which="btn", indices=[(2, 0)], color=HL_COLOR_2)
        self.wait(2)
        show_subtitle(self, "这是一个十分重要的性质。", "通过这个性质，可以由第一个灯推导后面的灯。")
        self.wait(2)
...
#【演示】圈出左边两个矩阵，然后再圈出第三矩阵
        ...
        LAT1_2 = show_latex(self, "<cY>B(n,x-1,y)⊕B(n,x+1,y)⊕B(n,x,y-1)⊕B(n,x,y+1)", 0, 2.0, show=False)
        trans_latex(self, LAT1_1, LAT1_2)
#【演示】圈出第三矩阵一个圈
        show_subtitle(self, "将刚才的矩阵递推关系加上参数y并重写。")
        LAT2_1 = show_latex(self, "<cP>B(n,x,y)=B(n-1,x-1,y)⊕B(n-1,x,y)⊕B(n-1,x+1,y)⊕B(n-2,x,y)", 0, 1.5)
        show_subtitle(self, "对于B(n)，我们将式子竖着写成四项，", "然后将递推关系代入这个式子。")
        LAT1_3 = show_latex(self, "<cY>B(n,x-1,y)⊕<br><cY>B(n,x+1,y)⊕<br><cY>B(n,x,y-1)⊕<br><cY>B(n,x,y+1)", 0, 1.5, show=False)
        trans_latex(self, LAT1_2, LAT1_3)
...
#【演示】依次画出左边的4各圈
        ...
        LAT1_6 = show_latex(self, "<cY>0⊕<br><cY>0⊕<br><cY>0⊕<br><cY>0", 0, 1.5, show=False)
...
【演示】圈出第一个矩阵
        show_subtitle(self, "第二个矩阵是其实也可以表示为左边两个矩阵的叠加。", "由于单位矩阵左边没有矩阵，因此相当于叠加了一个零矩阵。")

【演示】画出隐藏零矩阵并圈出
        show_subtitle(self, "由于零矩阵显然满足要求，因此第二个矩阵也满足要求。")
