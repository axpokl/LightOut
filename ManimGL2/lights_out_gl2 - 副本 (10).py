...
def hl_bd(scene, G, color=HL_COLOR, width=BD_W_SEL, buff=0.06, rt=0.3):
    if "extras" not in G:
        G["extras"] = {}
    frames = []
    lgt_frame = (
...
def hl_cells(scene, grids, which="lgt", indices=[(0,0)], color=HL_COLOR, width=BD_W_SEL, rt=0.3):
...
MAT4K = make_mat_k(4)
MAT5K = make_mat_k(5)
MAT3KY = make_mat_ky(3)
MAT4KY = make_mat_ky(4)
...
#演示n=5,7,11
        ...
        LAT1 = show_latex(self, "<cP>B(n,x)=B(n-1,x-1)⊕B(n-1,x)⊕B(n-1,x+1)⊕B(n-2,x)", 0, 2.0)
        LAT2 = show_latex(self, "<cP>B按钮    <cB>L灯    <cR>⊕叠加    <cY>~翻转", 0, 1.5)
        self.wait(4)
        ...
        show_subtitle(self, "n代表从上到下第n行灯或按钮，也代表第n次迭代，", "因为我们是一行行进行推导的。")
        self.wait(4)
        show_subtitle(self, "x代表从左到右第x列按钮。将灯表示为第一行的某几个列的按钮的叠加。", "这里的x不局限于第一行。")
        self.wait(4)
        show_subtitle(self, "y代表从左到右第y列灯。", "这里的公式对任意第y个灯都满足，所以省去。")
        self.wait(4)

        show_subtitle(self, "在上集视频中，因为最终目标是将灯用按钮表达，", "因此省去了按钮表达按钮，粉色原点直接为按钮表达灯。")
        self.wait(4)
        show_subtitle(self, "在这里，粉色原点是用按钮表达按钮，淡蓝色方块是用按钮表达灯。")
        self.wait(4)
        ...
        self.wait(2)
        show_subtitle(self, "粉色圆圈代表第y列的按钮是由第一行的哪几个按钮叠加的。", "淡蓝色方块代表第y列的灯是由第一行的哪几个按钮叠加的。")
        self.wait(4)
        show_subtitle(self, "由于按法是动态的，当第二行按钮还未被按下时，", "L1=B1⊕B2，即第一个灯L1由第一行的一、二个按钮B1,B2叠加。")
        ...
        show_subtitle(self, "然后，B6=~L1=~(B1⊕B2)，即第六个按钮B6是第一个灯L1的翻转。", "因此，第二行的按钮B6相和第一行灯L1的叠加状态是相同的。")
        toggle_btn(self, G5_[1][0], 0, 0)
        toggle_btn(self, G5_[1][0], 1, 0)
        self.wait(4)

        show_subtitle(self, "同理，B7=~L2=~(B1⊕B2⊕B3)，即第七个按钮B6是第二个灯L2的翻转。", "同样，第二行的按钮BL相和第一行灯L2的叠加状态是相同的。")
        ...
        show_subtitle(self, "接下来我们来看灯第六个灯。", "L6=B1⊕B6⊕B7=B1⊕~(B1⊕B2)⊕~(B1⊕B2⊕B3)=B1⊕B3。")
        ...
        add_cell(self, G5_[0][0], G5_[1][0], 0, 0, 0, 0, color_from=RED, color_to=YELLOW, rt=0.6)

        add_cell(self, G5_[1][0], G5_[1][0], 0, 0, 0, 0, color_from=RED, color_to=YELLOW, rt=0.6)
        add_cell(self, G5_[1][0], G5_[1][0], 1, 0, 1, 0, color_from=RED, color_to=YELLOW, rt=0.6)

        add_cell(self, G5_[1][1], G5_[1][0], 0, 0, 0, 0, color_from=RED, color_to=YELLOW, rt=0.6)
        add_cell(self, G5_[1][1], G5_[1][0], 1, 0, 1, 0, color_from=RED, color_to=YELLOW, rt=0.6)
        add_cell(self, G5_[1][1], G5_[1][0], 2, 0, 2, 0, color_from=RED, color_to=YELLOW, rt=0.6)
        ...
        add_cell(self, G5_[0][1], G5_[1][1], 1, 0, 1, 0, color_from=RED, color_to=YELLOW, rt=0.6)
        add_cell(self, G5_[1][0], G5_[1][1], 0, 0, 0, 0, color_from=RED, color_to=YELLOW, rt=0.6)
        add_cell(self, G5_[1][0], G5_[1][1], 1, 0, 1, 0, color_from=RED, color_to=YELLOW, rt=0.6)
        add_cell(self, G5_[1][1], G5_[1][1], 0, 0, 0, 0, color_from=RED, color_to=YELLOW, rt=0.6)
        add_cell(self, G5_[1][1], G5_[1][1], 1, 0, 1, 0, color_from=RED, color_to=YELLOW, rt=0.6)
        add_cell(self, G5_[1][1], G5_[1][1], 2, 0, 2, 0, color_from=RED, color_to=YELLOW, rt=0.6)
        add_cell(self, G5_[1][2], G5_[1][1], 1, 0, 1, 0, color_from=RED, color_to=YELLOW, rt=0.6)
        add_cell(self, G5_[1][2], G5_[1][1], 2, 0, 2, 0, color_from=RED, color_to=YELLOW, rt=0.6)
        add_cell(self, G5_[1][2], G5_[1][1], 3, 0, 3, 0, color_from=RED, color_to=YELLOW, rt=0.6)
        ...
        LAT1_1 = show_latex(self, "<cB>L(n,x,y)=<cP>B(n,x,y-1)⊕B(n,x,y)⊕B(n,x,y+1)⊕B(n-1,x,y)", 0, 2.0)
        top_objs = [[None] * cols for _ in range(rows)]
        topy_objs = [[None] * cols for _ in range(rows)]
        left_objs = [[None] * cols for _ in range(rows)]
        for k in range(rows):
        ...
        LAT0_1 = show_latex(self, "<cY>B(n,x-1,y)⊕B(n,x+1,y)⊕B(n,x,y-1)⊕B(n,x,y+1)=0", 0, 2.5)
        ...
        LAT0_2 = show_latex(self, "<cY>B(n,x-1,y)⊕B(n,x+1,y)=B(n,x,y-1)⊕B(n,x,y+1)", 0, 2.5, show=False)
        ...
        hl_cells(self, [G5_[1][2]], which="btn", indices=[(1, 0)])
        hl_cells(self, [G5_[0][1]], which="btn", indices=[(1, 0)])
        LAT1_2 = show_latex(self, "<cB>L(n,x,y)=<cP>B(n,x-1,y)⊕B(n,x,y)⊕B(n,x+1,y)⊕B(n-1,x,y)", 0, 2.0, show=False)
        ...
        LAT1_3 = show_latex(self, "<cB>L(n,x)=<cP>B(n,x-1)⊕B(n,x)⊕B(n,x+1)⊕B(n-1,x)", 0, 2.0, show=False)
        trans_latex(self, LAT1_2, LAT1_3)
        self.wait(4)

        show_subtitle(self, "又因为，下一行按钮是当前灯翻转。")
        LAT1_4 = show_latex(self, "<cP>B(n+1,x)=<cB>~L(n,x)=<cP>~(B(n,x-1)⊕B(n,x)⊕B(n,x+1)⊕B(n-1,x))", 0, 2.0, show=False)
        trans_latex(self, LAT1_3, LAT1_4)
        self.wait(2)
        show_subtitle(self, "以及，下一行灯是上左中右灯翻转的叠加。")
        LAT2_1 = show_latex(self, "<cB>L(n+1,x)=<cB>~L(n,x-1)⊕~L(n,x)⊕~L(n,x+1)⊕~L(n-1,x)", 0, 1.5)
        self.wait(2)
        ...
        LAT3 = show_latex(self, "<cY>Y(n,y)=<cY>~(Y(n-1,y-1)⊕Y(n-1,y)⊕Y(n-1,y+1)⊕Y(n-2,y))", 0, 1.5)
        self.wait(2)
