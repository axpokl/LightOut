...
def show_latex(scene, text, x=0.0, y=0.0, run_in=0.3, run_out=0.3, font=DEFAULT_FONT, font_size=32, buff=0.5, baseline=0.05, auto_k=0.5, ref_tex="N\\times N", show=True):
    parts = []
    if isinstance(text, (list, tuple)):
...
#演示n=5,7,11
        ...
        LAT1 = show_latex(self, "B(n,x)=B(n-1,x-1)⊕B(n-1,x)⊕B(n-1,x+1)⊕B(n-2,x)", 0, 2)
        LAT2 = show_latex(self, "B按钮    L灯    ⊕叠加    ~翻转", 0, 1.5)
        self.wait(4)
