...
#——————————————————————
        ...
        show_subtitle(self, "现在，如果我们把序号1-25用坐标x,y表示，则灯和按钮矩阵可写为以下公式：")
        LAT1 = show_latex(self, "L(n,x)=B(n-1,x-1)⊕B(n,x-1)⊕B(n,x)⊕B(n,x+1)", 0, 2.5)
        show_subtitle(self, "灯为上左中右按钮的叠加。")
        LAT2 = show_latex(self, "B(n+1,x)=~L(n,x)=~(B(n-1,x-1)⊕B(n,x-1)⊕B(n,x)⊕B(n,x+1))", 0, 2.0)
