...
#演示n=5,7,11
        ...
        show_subtitle(self, "n代表从上到下第n行灯或按钮，也代表第y次迭代，", "因为我们是一行行进行推导的。")
#圈出n1-5,然后消失
        show_subtitle(self, "x代表从左到右第x列按钮。将灯表示为第一行的某几个列的按钮的叠加。", "这里的x不局限于第一行。")
#圈出x1-5,然后消失
        show_subtitle(self, "y代表从左到右第y列灯。", "这里的公式对任意第y个灯都满足，所以省去。")

        show_subtitle(self, "这里，粉色原点是用按钮表达按钮，淡蓝色方块是用按钮表达灯。")
        show_subtitle(self, "上集视频中，因为最终目标是将灯用按钮表达，", "因此省去了按钮表达按钮，粉色原点直接为按钮表达灯。")
        ...
        toggle_lgt(self, G5_[1][1], 1, 0)
        del_cells(self, [G5_[0][1]], which="btn", indices=[(1, 0)])
        toggle_lgt(self, G5_[1][1], 0, 0)
        del_cells(self, [G5_[1][0]], which="btn", indices=[(0, 0)])
        toggle_lgt(self, G5_[1][1], 1, 0)
        del_cells(self, [G5_[1][0]], which="btn", indices=[(1, 0)])
        toggle_lgt(self, G5_[1][1], 0, 0)
...
#——————————————————————
        """
        show_subtitle(self, "现在，如果我们把序号1-25用坐标x,y表示，则灯和按钮矩阵可写为这个公式：")
        LAT1 = show_latex(self, "L(n,x,y)=B(n-1,x,y)⊕B(n,x,y-1)⊕B(n,x,y)⊕B(n,x,y+1)", 0, 2.5)
        show_subtitle(self, "这里，我们的矩阵同时满足另一个性质，", "我称之为十字偶校验约束。")
        LAT1 = show_latex(self, "B(n,x-1,y)⊕B(n,x+1,y)⊕B(n,x,y-1)⊕B(n,x,y+1)=0", 0, 2.5)
        show_subtitle(self, "某个元素的左右两个矩阵的对应位置的元素，", "和这个元素上下两个元素的叠加后为零。")
        LAT1 = show_latex(self, "B(n,x-1,y)⊕B(n,x+1,y)=B(n,x,y-1)⊕B(n,x,y+1)", 0, 2.5)
        show_subtitle(self, "这就是之前视频优化生成矩阵章节中的性质，", "后面我会给予证明。")
        LAT1 = show_latex(self, "L(n,x,y)=B(n-1,x,y)⊕B(n,x-1,y)⊕B(n,x,y)⊕B(n,x+1,y)")
