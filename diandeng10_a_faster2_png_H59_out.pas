...
var yp,yq:TVec;
begin
BuildFCPairsFast(n,f,c,f1,c1,hf,hc,hf1,hc1);
...
p0:=0;
while p0<=common-3 do
  begin
  ar[p0+sh]:=ar[p0+sh] xor br[p0];
  av[p0+sh]:=av[p0+sh] xor bv[p0];
  ar[p0+sh+1]:=ar[p0+sh+1] xor br[p0+1];
  av[p0+sh+1]:=av[p0+sh+1] xor bv[p0+1];
  ar[p0+sh+2]:=ar[p0+sh+2] xor br[p0+2];
  av[p0+sh+2]:=av[p0+sh+2] xor bv[p0+2];
  ar[p0+sh+3]:=ar[p0+sh+3] xor br[p0+3];
  av[p0+sh+3]:=av[p0+sh+3] xor bv[p0+3];
  inc(p0,4);
  end;
while p0<=common do
  begin
...
lenBits:=lenWords shl 5;
SetLength(aa,lenBits); SetLength(bb,lenBits);
for p:=0 to dv do aa[p]:=vv[p];
for p:=0 to db do bb[p]:=vb[p];
KarMul(aa,bb,prod,lenWords,dv+1,db+1);
...
var halfLen,period,convWords,convBits,p,i0,center,lo,hi0:longint;
var combo,ha,prod,prefix:TDynBool;
var bit0:boolean;
begin
halfLen:=hi+2;
...
SetLength(prefix,halfLen+1);
prefix[0]:=ha[0];
for p:=1 to halfLen do prefix[p]:=prefix[p-1] xor ha[p];
SetLength(prod,period+1);
for p:=0 to period do
  ...
  if lo<=hi0 then
    begin
    prod[p]:=prefix[hi0];
    if lo>0 then prod[p]:=prod[p] xor prefix[lo-1];
    end;
  end;
for i0:=0 to hi do
  begin
  p:=i0+1;
  bit0:=prod[p] xor prod[period-p] xor prod[halfLen-p] xor prod[halfLen+p] xor
        ha[0] xor ha[halfLen];
  vdst[i0]:=bit0;
  end;
...
var i0,r0,rU,kk,jmax,target,high:longint;
...
if jmax>=0 then
  ...
  if jmax>=r0 then
    begin
    write('K ');for kk:=0 to r0 shl 1 do if pg1^[kk] then write(1) else write(0);writeln;
    DivMonicBlock(z,pg1^,v,longint(n)-1,r0 shl 1);
    write('Q ');for kk:=0 to jmax-r0 do if v[kk] then write(1) else write(0);writeln;
