...
{ H62 revised: repeated Fibonacci reduction before the H61 Euclid core. }
{ A/B share the reduction and lifting formulas; B expands them by LongWord. }
...
{ Repeated Fibonacci reduction.  The irreducible D-family is still
  solved by the H61 Euclid kernel.  All reconstruction is charged to q. }
...
if jmax>=0 then
  ...
  if jmax>=r0 then
    begin
    write('K ');for i0:=0 to r0 shl 1 do if pg1^[i0] then write(1) else write(0);writeln;
    DivMonicBlock(z,pg1^,v,longint(n)-1,r0 shl 1);
    write('Q ');for i0:=0 to jmax-r0 do if v[i0] then write(1) else write(0);writeln;
