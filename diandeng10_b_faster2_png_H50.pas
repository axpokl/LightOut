...
{ H50: keep only the independent half of each palindromic y during MakeMat. }
{ A and B use the same formula and recurrence; only bit storage differs. }
...
procedure ReverseVector(const src:TVec; var dst:TVec; bits:longint);
var words,w,startP:longint;
begin
words:=(bits+31) shr 5;
for w:=0 to words-1 do
  begin
  startP:=w shl 5;
  dst[w]:=ReverseWord32(ReadVec32Any(src,bits-startP-32));
  end;
if (bits and 31)<>0 then dst[words-1]:=dst[words-1] and ((LongWord(1) shl (bits and 31))-1);
for w:=words to wn-1 do dst[w]:=0;
VecNorm(dst);
end;
...
if jmax=0 then begin VecCopy(pg1^,pg0^); VecZero(pg2^); end
  ...
  VecXorEq(v,v2);
  MaskDeg(v,n-1);
  ReverseVector(pg0^,pg1^,longint(n));
  ReverseVector(v,pg2^,longint(n));
  for j:=1 to r0 do
    begin
    VecShiftL1(v1,pg2^);
    VecShiftR1(v2,pg2^);
