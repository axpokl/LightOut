...
type TVec=array[-2..m]of boolean;
     ...
     TBmpFileHeader=packed record
       bfType:Word; bfSize:LongWord; bfReserved1:Word; bfReserved2:Word; bfOffBits:LongWord;
     end;
     TBmpInfoHeader=packed record
       biSize:LongWord; biWidth:LongInt; biHeight:LongInt; biPlanes:Word; biBitCount:Word;
       biCompression:LongWord; biSizeImage:LongWord; biXPelsPerMeter:LongInt; biYPelsPerMeter:LongInt;
       biClrUsed:LongWord; biClrImportant:LongWord;
     end;
...
fh.bfType:=$4D42; fh.bfOffBits:=SizeOf(fh)+SizeOf(ih)+2*SizeOf(TRGBQuad); fh.bfReserved1:=0; fh.bfReserved2:=0;
fh.bfSize:=fh.bfOffBits+LongWord(bw.rowPad)*LongWord(height);
ih.biSize:=SizeOf(ih); ih.biWidth:=width; ih.biHeight:=-height; ih.biPlanes:=1; ih.biBitCount:=1; ih.biCompression:=0;
ih.biSizeImage:=LongWord(bw.rowPad)*LongWord(height); ih.biXPelsPerMeter:=3780; ih.biYPelsPerMeter:=3780; ih.biClrUsed:=2; ih.biClrImportant:=2;
...
begin
outDir:=IncludeTrailingPathDelimiter(ExtractFilePath(ExpandFileName(ParamStr(0)))+'png_a_H28_T10000');
