...
//writeln('d');
//write(0,#9);for i:=0 to n-1 do if GetBit(d0,i)<>0 then write(1) else write(0);writeln;
if jmax=0 then VecCopy(d1,d0)
  ...
  for j:=1 to jmax do
    ...
    VecXorEq(d1,d2);
    MaskDeg(d1,n-1);
//  write(j,#9);for i:=0 to n-1 do if GetBit(d1,i)<>0 then write(1) else write(0);writeln;
    d2:=d0;
    d0:=d1;
    end;
  d1:=d0;
...
//for kk:=0 to n-1 do if GetBit(z,kk)<>0 then write(1) else write(0);write(#9);
    VecXorEq(z,d1);
    SetBit(x,i0,1);
//for kk:=0 to n-1 do if GetBit(d1,kk)<>0 then write(1) else write(0);write(#9);
//for kk:=0 to n-1 do if GetBit(z,kk)<>0 then write(1) else write(0);write(#9);
