...
procedure KarRecPair(const a:TDynBool; ao:longint;
                     ...
                     var s:TDynBool; so,len,alen,blen,clen:longint;
...
if (a1len=0) and (b1len=0) and (c1len=0) then
  begin
  KarRecPair(a,ao,b,bo,c,co,r,ro,s,so,h,a0len,b0len,c0len,work,wo);
  exit;
  end;
KarRecPair(a,ao,b,bo,c,co,r,ro,s,so,h,a0len,b0len,c0len,work,wo);
KarRecPair(a,ao+h,b,bo+h,c,co+h,r,ro+(h shl 1),s,so+(h shl 1),h,
...
KarRecPair(a,0,b,0,c,0,r,0,s,0,len,alen,blen,clen,work,0);
end;
...
KarMulPair(ha,hb,hbr,prod0,prod1,halfPad,halfLen+1,halfLen,halfLen);
for i0:=0 to hi do
  begin
  p:=i0+1;
