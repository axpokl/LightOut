...
begin
BMP1Open(bw,fn,n,n);
for col:=-2 to longint(n) do begin x2[col]:=false; x1[col]:=false; end;
for col:=0 to longint(n)-1 do x1[col]:=x[col];
BMP1WriteVecRow(bw,x1,n);
for rowIndex:=1 to longint(n)-1 do
  begin
  x0[-2]:=false; x0[-1]:=false; x0[n]:=false;
  for col:=0 to longint(n)-1 do
    x0[col]:=not(x1[col-1] xor x1[col] xor x1[col+1] xor x2[col]);
  BMP1WriteVecRow(bw,x0,n);
  for col:=-2 to longint(n) do begin x2[col]:=x1[col]; x1[col]:=x0[col]; end;
  end;
BMP1Close(bw);
WriteFullBMPAndCheck:=true;
for col:=0 to longint(n)-1 do
  WriteFullBMPAndCheck:=WriteFullBMPAndCheck and
    (x1[col-1] xor x1[col] xor x1[col+1] xor x2[col]);
end;
...
begin
outDir:=IncludeTrailingPathDelimiter(ExtractFilePath(ExpandFileName(ParamStr(0)))+'png_a_H64_T10000');
