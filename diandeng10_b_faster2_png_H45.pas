...
{ H45: skip zero-padded branches in the paired Karatsuba recursion. }
...
procedure KarRecPairW(const a:TWordArray; ao:longint;
                      ...
                      var s:TWordArray; so,len,alen,blen,clen:longint;
                      var work:TWordArray; wo:longint);
const cut=4;
var i0,j0,h,ax0,bx0,cx0,zr0,zs0,saver0,saves0,rec0:longint;
var a0len,a1len,b0len,b1len,c0len,c1len,axlen,bxlen,cxlen,maxbc:longint;
var p0:QWord;
var tr,ts:LongWord;
begin
if alen>len then alen:=len;
if blen>len then blen:=len;
if clen>len then clen:=len;
if (alen<=0) or ((blen<=0) and (clen<=0)) then exit;
if len<=cut then
  begin
  maxbc:=blen; if clen>maxbc then maxbc:=clen;
  for i0:=0 to alen-1 do if a[ao+i0]<>0 then
    for j0:=0 to maxbc-1 do
      begin
      if (j0<blen) and (b[bo+j0]<>0) then
        begin
        p0:=CLMul32(a[ao+i0],b[bo+j0]);
        r[ro+i0+j0]:=r[ro+i0+j0] xor LongWord(p0);
        r[ro+i0+j0+1]:=r[ro+i0+j0+1] xor LongWord(p0 shr 32);
        end;
      if (j0<clen) and (c[co+j0]<>0) then
        begin
...
if (a1len=0) and (b1len=0) and (c1len=0) then
  begin
  KarRecPairW(a,ao,b,bo,c,co,r,ro,s,so,h,a0len,b0len,c0len,work,wo);
  exit;
  end;
KarRecPairW(a,ao,b,bo,c,co,r,ro,s,so,h,a0len,b0len,c0len,work,wo);
KarRecPairW(a,ao+h,b,bo+h,c,co+h,r,ro+(h shl 1),s,so+(h shl 1),h,
...
KarRecPairW(work,ax0,work,bx0,work,cx0,
            work,zr0,work,zs0,h,axlen,bxlen,cxlen,work,rec0);
for i0:=0 to h-1 do
  begin
...
procedure KarMulPairFastW(const a,b,c:TWordArray; var r,s:TWordArray;
                          len,alen,blen,clen:longint);
var work:TWordArray;
var i0:longint;
begin
SetLength(r,len shl 1);
SetLength(s,len shl 1);
for i0:=0 to High(r) do
  begin
  r[i0]:=0;
  s[i0]:=0;
  end;
SetLength(work,len*10);
KarRecPairW(a,0,b,0,c,0,r,0,s,0,len,alen,blen,clen,work,0);
end;
...
var activeAWords,activeBWords:longint;
...
activeAWords:=(halfLen+32) shr 5;
activeBWords:=(halfLen+31) shr 5;
KarMulPairFastW(ha,hb,hbr,prod0,prod1,halfWords,
                activeAWords,activeBWords,activeBWords);
VecZero(vdst);
lastWord:=hi shr 5;
