...
{ H70: combine adjacent quotient terms in Euclidean leaf updates. }
...
procedure XS2(var x:TL; const a:TL; sh,d:longint); inline;
var j,ws,bs,last:longint;
var v:QWord; carry:LongWord;
begin
if d<0 then exit;
ws:=sh shr 5; bs:=sh and 31; last:=d shr 5; carry:=0;
for j:=0 to last do
  begin
  v:=(QWord(a[j]) shl bs) xor (QWord(a[j]) shl (bs+1));
  x[ws+j]:=x[ws+j] xor LongWord(v) xor carry;
  carry:=LongWord(v shr 32);
  end;
x[ws+last+1]:=x[ws+last+1] xor carry;
end;
...
while (d1>=0) and (d1>=target) do
  ...
  while d0>=d1 do
    ...
    if (sh>0) and (d1>0) and ((((r0^[(d0-1) shr 5] shr ((d0-1) and 31)) xor (r1^[(d1-1) shr 5] shr ((d1-1) and 31))) and 1)<>0) then
      begin
      XS2(r0^,r1^,sh-1,d1); XS2(u0^,u1^,sh-1,du1); XS2(v0^,v1^,sh-1,dv1);
      dec(d0,2);
      end
    else
      begin
...
KarMulPairFastW(ha,hb,hbr,prod0,prod1,convWords,halfLen+1,halfLen,halfLen);
VecZero(vdst);
lastWord:=hi shr 5;
for w0:=0 to lastWord do
  begin
