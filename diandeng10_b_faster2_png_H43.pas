...
{ H43: the same Laurent-kernel and exact NTT algorithm as A for every n. }
{ Only GF(2) storage and access are packed into LongWord. }
...
for w0:=0 to wn-1 do
  ...
  while q0<>0 do
    ...
    if i0<=hi then
      begin
      p0:=i0+1;
      src[p0 shr 5]:=src[p0 shr 5] xor (LongWord(1) shl (p0 and 31));
      p0:=period-i0-1;
      src[p0 shr 5]:=src[p0 shr 5] xor (LongWord(1) shl (p0 and 31));
      end;
    q0:=q0 and (q0-1);
...
NTTMulW(kernel,src,prod,pad);
VecZero(vdst);
for i0:=0 to hi do
  begin
  bit0:=0;
  p0:=i0+1;
  while p0<(pad shl 1) do
    begin
    bit0:=bit0 xor DynBit(prod,p0);
    inc(p0,period);
    end;
